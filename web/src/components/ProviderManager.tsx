...
function ProviderCard({ draft, update, onSave, onDiscover, onDelete }: { draft: Draft; update: (patch: Partial<Draft>) => void; onSave: () => void; onDiscover: () => void; onDelete: () => void }) {
  ...
  const nameId = `provider-name-${draft.id}`
  const keyId = `provider-key-${draft.id}`
  const baseUrlId = `provider-base-url-${draft.id}`
  const modelsId = `models-${draft.id}`

  return <article className="provider-card">
    <div className="provider-card-head">
      <div>
        <span className="provider-index">PROVIDER</span>
        <h2>{draft.name || 'New provider'}</h2>
      </div>
      {draft.id && <button className="icon-button" type="button" aria-label="删除 provider" onClick={onDelete}>×</button>}
    </div>

    <div className="provider-form">
      <Field id={nameId} label="Provider name">
        <input id={nameId} className="field-input" value={draft.name} onChange={event => update({ name: event.target.value })} placeholder="OpenRouter / LM Studio / My API" />
      </Field>

      <Field id={keyId} label="API key">
        <div className="secret-input">
          <input id={keyId} className="field-input" type="password" value={draft.apiKey} onChange={event => update({ apiKey: event.target.value })} placeholder={draft.hasApiKey ? '已保存，留空表示不修改' : 'sk-…'} />
          <span className="field-status" aria-live="polite">{draft.hasApiKey ? 'SAVED' : 'NEW'}</span>
        </div>
      </Field>

      <Field id={baseUrlId} label="Custom base URL">
        <input id={baseUrlId} className="field-input" value={draft.baseUrl} onChange={event => update({ baseUrl: event.target.value })} placeholder="https://api.example.com/v1" />
      </Field>
    </div>
    ...
    {draft.error && <p className="form-message form-message-error" role="alert">{draft.error}</p>}

    <section className="models-section" aria-labelledby={modelsId}>
      <div className="models-head">
        ...
        <div className="select-actions">
          ...
          <button className="select-button" type="button" aria-pressed={draft.models.length > 0 && draft.selectedModelIds.length === 0} onClick={() => setAll(false)} disabled={!draft.models.length}>取消全部</button>
        </div>
      </div>

      {draft.models.length > 0 ? <div className="model-list">{draft.models.map(model => <label className="model-row" key={model.id}>
        <input className="model-checkbox" type="checkbox" checked={selected.has(model.id)} onChange={() => toggle(model.id)} />
